...
from Queries import CriteriaProperties
...
def create_various_queries(asm):
    ...
    # "STEEL PLATE"
    steel_plate = CriteriaProperties(
    ...
    ss_plate = CriteriaProperties(
        seQueryPropertyCustom, "JDEDSC1_A", seQueryConditionContains, "SS.PLATE"
    )
    create_query(asm.Queries, "Material [SS.PLATE]", [ss_plate.criterias])

    # "ALU"
    alu_plate = CriteriaProperties(
        seQueryPropertyCustom, "JDEDSC1_A", seQueryConditionContains, "AL.PLATE"
    )
    create_query(asm.Queries, "Material [AL.PLATE]", [alu_plate.criterias])
